...
def update_answer(answer: str, identity: int):
    a = QuestionAnswer.query.filter(QuestionAnswer.id == identity).one()
    a.answer = answer
    db.session.commit()
    return a
...
def save_answer():
    ...
    return jsonify(update_answer(answer=answer, identity=question_id))
